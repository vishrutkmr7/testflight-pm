...
function parseActionInputs(): ActionInputs {
    return {
        createGithubIssues: process.env.INPUT_CREATE_GITHUB_ISSUES?.toLowerCase() === 'true',
        createLinearIssues: process.env.INPUT_CREATE_LINEAR_ISSUES?.toLowerCase() === 'true',
        monitorSince: process.env.INPUT_MONITOR_SINCE,
        maxIssuesPerRun: parseInt(process.env.INPUT_MAX_ISSUES_PER_RUN || '10', 10),
        feedbackTypes: (process.env.INPUT_FEEDBACK_TYPES as any) || 'all',
        issueLabels: (process.env.INPUT_ISSUE_LABELS || 'testflight,feedback').split(',').map(l => l.trim()),
        crashIssueLabels: (process.env.INPUT_CRASH_ISSUE_LABELS || 'bug,crash').split(',').map(l => l.trim()),
        feedbackIssueLabels: (process.env.INPUT_FEEDBACK_ISSUE_LABELS || 'enhancement,user-feedback').split(',').map(l => l.trim()),
        duplicateDetection: process.env.INPUT_DUPLICATE_DETECTION?.toLowerCase() !== 'false',
        includeDeviceInfo: process.env.INPUT_INCLUDE_DEVICE_INFO?.toLowerCase() !== 'false',
        includeAppVersion: process.env.INPUT_INCLUDE_APP_VERSION?.toLowerCase() !== 'false',
...
function getMonitoringSince(inputs: ActionInputs): Date {
    if (inputs.monitorSince) {
        try {
            ...
            setWarning(`Invalid monitor-since date: ${inputs.monitorSince}. Using default (24 hours ago).`);
        }
    }
 ...
 */
function filterFeedback(allFeedback: ProcessedFeedbackData[], inputs: ActionInputs): ProcessedFeedbackData[] {
    ...
    if (filtered.length > inputs.maxIssuesPerRun) {
        setWarning(`Found ${filtered.length} feedback items, but limiting to ${inputs.maxIssuesPerRun} per action run.`);
 ...
 */
function generateIssueTitle(feedback: ProcessedFeedbackData, inputs: ActionInputs): string {
 ...
 */
async function createGitHubIssue(feedback: ProcessedFeedbackData, inputs: ActionInputs): Promise<boolean> {
    if (inputs.dryRun) {
        setInfo(`[DRY RUN] Would create GitHub issue: ${generateIssueTitle(feedback, inputs)}`);
        return true;
    }

    // TODO: Implement GitHub Issues API integration (VJ-17)
    setWarning('GitHub Issues integration not yet implemented. Issue creation skipped.');
    return false;
}
 ...
 */
async function createLinearIssue(feedback: ProcessedFeedbackData, inputs: ActionInputs): Promise<boolean> {
    if (inputs.dryRun) {
        setInfo(`[DRY RUN] Would create Linear issue: ${generateIssueTitle(feedback, inputs)}`);
        return true;
    }
    ...
    try {
        ...
        if (!validateLinearConfig()) {
            setWarning('Linear integration not configured. Please set LINEAR_API_TOKEN and LINEAR_TEAM_ID.');
            return false;
        }
...
async function runAction(): Promise<ActionOutputs> {
    ...
    try {
        ...
        setInfo(`📊 Configuration: ${inputs.feedbackTypes} feedback since ${since.toISOString()}`);
        setInfo(`🎯 Targets: GitHub=${inputs.createGithubIssues}, Linear=${inputs.createLinearIssues}`);
        ...
        if (inputs.createLinearIssues) {
            ...
            if (linearHealth.status === 'unhealthy') {
                setError(`Linear integration is unhealthy: ${JSON.stringify(linearHealth.details)}`);
                if (linearHealth.recommendations) {
                    linearHealth.recommendations.forEach(rec => setWarning(`💡 Recommendation: ${rec}`));
                }
                outputs.errorsEncountered++;
                return outputs;
            } else if (linearHealth.status === 'degraded') {
                setWarning(`Linear integration is degraded: ${JSON.stringify(linearHealth.details)}`);
                if (linearHealth.recommendations) {
                    linearHealth.recommendations.forEach(rec => setWarning(`💡 Recommendation: ${rec}`));
                }
            } else {
                setInfo(`✅ Linear integration healthy: ${linearHealth.details.teamName} (${linearHealth.details.currentUser})`);
            }
        }
        ...
        const screenshots = feedbackToProcess.filter(f => f.type === 'screenshot');
        outputs.crashesProcessed = crashes.length;
        outputs.feedbackProcessed = screenshots.length;

        setInfo(`📋 Breakdown: ${crashes.length} crash reports, ${screenshots.length} user feedback items`);
        ...
        setInfo(`📊 Summary: ${outputs.issuesCreated} issues created, ${outputs.errorsEncountered} errors`);
