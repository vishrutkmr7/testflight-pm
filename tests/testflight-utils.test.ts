...
import { getConfig, clearConfigCache } from '../src/config/environment.js';
import { getAuthInstance, clearAuthInstance } from '../src/api/app-store-connect-auth.js';
import { getTestFlightClient, clearClientInstance } from '../src/api/testflight-client.js';
    ...
    it('should process crash reports correctly', () => {
        ...
        const mockCrashReport = {
            ...
            attributes: {
                ...
                crashLogs: [{ url: 'https://example.com/log', expiresAt: '2023-01-02T12:00:00Z' }],
                deviceFamily: 'iPhone',
...
-----END PRIVATE KEY-----`;
            ...
            try {
                ...
                expect(errorMessage).not.toContain('SuperSecretKeyContentThatShouldNeverBeLogged');
            }
            ...
            expect(allErrors).not.toContain('SuperSecretKeyContentThatShouldNeverBeLogged');
        } finally {
            console.error = originalError;
        }
    });
    ...
    it('should validate environment variable presence', () => {
        ...
        expect(() => getConfig()).toThrow('Required environment variable APP_STORE_CONNECT_ISSUER_ID is not set or is empty');
    });

    it('should validate private key format strictly', () => {
        // Test various invalid formats
        const invalidFormatTests = [
            { key: 'just-a-string', expectedError: 'Invalid private key format. Must be a PEM formatted private key.' },
            { key: 'BEGIN PRIVATE KEY\ncontent\nEND PRIVATE KEY', expectedError: 'Invalid private key format. Must be a PEM formatted private key.' },
            { key: '-----BEGIN CERTIFICATE-----\ncontent\n-----END CERTIFICATE-----', expectedError: 'Invalid private key format. Must be a PEM formatted private key.' },
            { key: '-----BEGIN PRIVATE KEY-----\n-----END PRIVATE KEY-----', expectedError: 'Invalid private key format. Private key appears to be empty.' },
        ];
        ...
        expect(() => getConfig()).toThrow('Either APP_STORE_CONNECT_PRIVATE_KEY or APP_STORE_CONNECT_PRIVATE_KEY_PATH must be set');
    });
});
