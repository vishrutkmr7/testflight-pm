...
import type { LinearIssue, LinearPriority, LinearIntegrationConfig } from '../../types/linear.js';
...
export async function createLinearIssueFromFeedback(
    ...
    if (!validateLinearConfig()) {
        throw new Error('Linear is not configured. Please set LINEAR_API_TOKEN and LINEAR_TEAM_ID.');
    }
    ...
    try {
        ...
        const wasExisting = options.enableDuplicateDetection !== false &&
...
export async function createLinearIssuesFromFeedbackBatch(
    ...
    for (let i = 0; i < feedbackItems.length; i += maxConcurrent) {
        ...
        results.push(...batchResults.filter(result => result !== null) as LinearIssueCreationResult[]);
 ...
 */
export function determineFeedbackPriority(feedback: ProcessedFeedbackData): LinearPriority {
    ...
    if (feedback.type === 'screenshot' && feedback.screenshotData?.text) {
        ...
        if (text.includes('feature') || text.includes('improvement') || text.includes('suggest') ||
            text.includes('would be nice') || text.includes('could you')) {
            return 4; // Low
        }
 ...
 */
export function formatFeedbackForLinear(feedback: ProcessedFeedbackData): { title: string; description: string } {
 ...
 */
export function validateLinearIntegration(): { valid: boolean; errors: string[] } {
    const errors: string[] = [];

    if (!validateLinearConfig()) {
        errors.push('Linear configuration missing. Please set LINEAR_API_TOKEN and LINEAR_TEAM_ID.');
    }
