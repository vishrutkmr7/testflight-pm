...
export async function createGitHubIssueFromFeedback(
    ...
    if (!validateGitHubConfig()) {
        throw new Error('GitHub is not configured. Please set GITHUB_TOKEN, GITHUB_OWNER, and GITHUB_REPO.');
    }
    ...
    try {
        ...
        throw new Error(`Failed to create GitHub issue from TestFlight feedback: ${error}`);
    }
}
...
export async function createGitHubIssuesFromFeedbackBatch(
    ...
    for (let i = 0; i < feedbackItems.length; i += maxConcurrent) {
        ...
        results.push(...batchResults.filter(result => result !== null) as GitHubIssueCreationResult[]);
 ...
 */
export function determineFeedbackPriority(feedback: ProcessedFeedbackData): GitHubPriority {
    ...
    if (feedback.type === 'crash') {
        ...
        if (feedback.crashData?.exceptionType?.toLowerCase().includes('out of memory') ||
            feedback.crashData?.exceptionType?.toLowerCase().includes('segmentation fault') ||
            feedback.crashData?.exceptionType?.toLowerCase().includes('access violation')) {
            return 'high';
        }
    ...
    if (feedback.type === 'screenshot' && feedback.screenshotData?.text) {
        ...
        if (text.includes('crash') || text.includes('broken') || text.includes('not working') ||
            text.includes('urgent') || text.includes('critical') || text.includes('blocking') ||
            text.includes('can\'t') || text.includes('cannot') || text.includes('won\'t') ||
            text.includes('error') || text.includes('bug')) {
            return 'high';
        }

        // Look for enhancement keywords
        if (text.includes('feature') || text.includes('improvement') || text.includes('suggest') ||
            text.includes('would be nice') || text.includes('could you') || text.includes('wish') ||
            text.includes('enhancement') || text.includes('add')) {
            return 'low';
        }
 ...
 */
export function generateFeedbackLabels(feedback: ProcessedFeedbackData): string[] {
    const labels: string[] = ['testflight'];

    // Type-based labels
    if (feedback.type === 'crash') {
        labels.push('bug', 'crash');
        ...
        if (feedback.screenshotData?.text) {
            ...
            if (text.includes('ui') || text.includes('interface') || text.includes('design') ||
                text.includes('layout') || text.includes('button') || text.includes('screen')) {
                labels.push('ui-ux');
            }

            if (text.includes('feature') || text.includes('improvement') || text.includes('enhancement') ||
                text.includes('add') || text.includes('would like') || text.includes('suggest')) {
                labels.push('enhancement');
            }

            if (text.includes('bug') || text.includes('broken') || text.includes('not working') ||
                text.includes('error') || text.includes('wrong') || text.includes('issue')) {
                labels.push('bug');
            }

            if (text.includes('performance') || text.includes('slow') || text.includes('lag') ||
                text.includes('freeze') || text.includes('hang')) {
                labels.push('performance');
            }

            if (text.includes('accessibility') || text.includes('a11y') || text.includes('screen reader') ||
                text.includes('voice over') || text.includes('contrast')) {
 ...
 */
export function formatFeedbackForGitHub(feedback: ProcessedFeedbackData): { title: string; body: string } {
 ...
 */
export function validateGitHubIntegration(): { valid: boolean; errors: string[] } {
    const errors: string[] = [];

    if (!validateGitHubConfig()) {
        errors.push('GitHub configuration missing. Please set GITHUB_TOKEN, GITHUB_OWNER, and GITHUB_REPO.');
    }
...
export async function createIssueFromTestFlightFeedback(
    ...
    if ((preferredPlatform === 'github' || preferredPlatform === 'both') && validateGitHubConfig()) {
        try {
            results.github = await createGitHubIssueFromFeedback(feedback, options.github);
        } catch (error) {
