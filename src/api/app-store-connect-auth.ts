...
export class AppStoreConnectAuth {
    ...
    private isTokenValid(): boolean {
        ...
        const refreshThreshold = new Date(
            this.currentToken.expiresAt.getTime() - (this.refreshThresholdMinutes * 60 * 1000)
        );
     ...
     */
    private async signJwt(payload: JwtTokenPayload, privateKey: string, keyId: string): Promise<string> {
        try {
            // JWT Header
            const header = {
                alg: 'ES256',
     ...
     */
    public getTokenInfo(): { isValid: boolean; expiresAt?: Date; issuedAt?: Date } {
        if (!this.currentToken) {
