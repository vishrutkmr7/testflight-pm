...
export class GitHubClient {
    ...
    constructor() {
        ...
        if (!envConfig.github) {
            throw new Error('GitHub configuration not found. Please set GITHUB_TOKEN, GITHUB_OWNER, and GITHUB_REPO.');
        }
    ...
    public async createIssueFromTestFlight(
        ...
        try {
            ...
            let attachmentResults: { uploaded: number; failed: number; details: Array<{ filename: string; success: boolean; error?: string; url?: string; }> } | undefined;
            if (options.attachScreenshots !== false && this.config.enableScreenshotUpload && issueData.attachments.length > 0) {
                attachmentResults = await this.uploadScreenshots(issueData.attachments as GitHubScreenshotUpload[], feedback);
     ...
     */
    public async createIssue(request: GitHubCreateIssueRequest): Promise<GitHubIssue> {
     ...
     */
    public async updateIssue(issueNumber: number, request: GitHubUpdateIssueRequest): Promise<GitHubIssue> {
        const response = await this.makeApiRequest<GitHubIssue>(
            'PATCH',
     ...
     */
    public async addCommentToIssue(issueNumber: number, body: string): Promise<GitHubComment> {
     ...
     */
    public async findDuplicateIssue(feedback: ProcessedFeedbackData): Promise<GitHubDuplicateDetectionResult> {
        try {
            ...
            const potentialDuplicate = searchResults.items.find(issue => {
                if (feedback.type === 'crash' && feedback.crashData?.exceptionType) {
                    return issue.title.includes(feedback.crashData.exceptionType) ||
                        issue.body?.includes(feedback.crashData.exceptionType);
                }
                if (feedback.screenshotData?.text) {
                    const feedbackWords = feedback.screenshotData.text.toLowerCase().split(' ');
                    const issueText = (issue.title + ' ' + (issue.body || '')).toLowerCase();
                    const matchingWords = feedbackWords.filter(word =>
                        word.length > 3 && issueText.includes(word)
                    ).length;
     ...
     */
    public async searchIssues(params: GitHubIssueSearchParams): Promise<GitHubSearchResponse<GitHubIssue>> {
        ...
        const response = await this.makeApiRequest<GitHubSearchResponse<GitHubIssue>>(
            'GET',
    ...
    public async getLabels(): Promise<GitHubLabel[]> {
        const cacheAge = this.lastCacheUpdate.labels ? Date.now() - this.lastCacheUpdate.labels.getTime() : Infinity;
    ...
    public async getMilestones(): Promise<GitHubMilestone[]> {
        const cacheAge = this.lastCacheUpdate.milestones ? Date.now() - this.lastCacheUpdate.milestones.getTime() : Infinity;
        const cacheExpiryMs = 5 * 60 * 1000; // 5 minutes
    ...
    private async uploadScreenshots(
        ...
        feedback: ProcessedFeedbackData
    ): Promise<{ uploaded: number; failed: number; details: Array<{ filename: string; success: boolean; error?: string; url?: string; }> }> {
        const results = {
            uploaded: 0,
            failed: 0,
            details: [] as Array<{ filename: string; success: boolean; error?: string; url?: string; }>,
        };
    ...
    private async getCurrentUser(): Promise<GitHubUser> {
        const response = await this.makeApiRequest<GitHubUser>(
            'GET',
            '/user'
        );
        return response.data;
    ...
    private async prepareIssueFromTestFlight(
        ...
        let body = options.customBody || this.formatIssueBody(feedback);

        // Determine labels
        const baseLabels = isCrash ? this.config.crashLabels : this.config.feedbackLabels;
        const allLabels = [...this.config.defaultLabels, ...baseLabels, ...(options.additionalLabels || [])];
            ...
            try {
                const screenshots = await testFlightClient.downloadScreenshots({
                    ...
                    attributes: {
                        ...
                        screenshots: feedback.screenshotData.images.map((img, index) => ({
                            url: img.url,
     ...
     */
    private formatScreenshotLinks(attachmentResults: { uploaded: number; failed: number; details: Array<{ filename: string; success: boolean; url?: string; }> }): string {
     ...
     */
    private async addTestFlightCommentToIssue(issueNumber: number, feedback: ProcessedFeedbackData): Promise<GitHubComment> {
