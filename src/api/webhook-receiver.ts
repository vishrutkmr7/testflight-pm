...
import type { TestFlightWebhookEvent, TestFlightCrashReport, TestFlightScreenshotFeedback } from '../../types/testflight.js';
...
export interface ProcessedWebhookEvent {
    eventType: 'BETA_FEEDBACK_CRASH_SUBMISSION' | 'BETA_FEEDBACK_SCREENSHOT_SUBMISSION';
    eventTime: Date;
    feedbackId: string;
...
export class TestFlightWebhookReceiver {
    ...
    constructor() {
        ...
        if (!this.webhookSecret) {
            console.warn('Webhook secret not configured. Signature verification will be skipped.');
        }
    }
     ...
     */
    public async handleWebhook(request: WebhookRequest): Promise<WebhookResponse> {
        try {
            // Validate request method
            if (request.method !== 'POST') {
                return this.errorResponse(405, 'Method not allowed. Only POST requests are accepted.');
            }
            ...
            if (this.webhookSecret) {
                const isValid = this.verifySignature(request.body, request.headers[this.signatureHeader]);
                if (!isValid) {
            ...
            console.log(`Successfully processed ${processedEvent.eventType} event for feedback ID: ${processedEvent.feedbackId}`);
     ...
     */
    private async processWebhookEvent(event: TestFlightWebhookEvent): Promise<ProcessedWebhookEvent> {
