...
export class LinearClient {
    ...
    constructor() {
        ...
        if (!envConfig.linear) {
            throw new Error('Linear configuration not found. Please set LINEAR_API_TOKEN and LINEAR_TEAM_ID.');
        }

        this.config = {
            apiToken: envConfig.linear.apiToken,
            teamId: envConfig.linear.teamId,
            defaultPriority: 3, // Normal priority
            defaultLabels: ['testflight', 'feedback'],
            crashLabels: ['bug', 'crash', 'urgent'],
            feedbackLabels: ['feature-request', 'user-feedback'],
    ...
    public async createIssueFromTestFlight(
        ...
        try {
            ...
            console.log(`✅ Created Linear issue: ${createdIssue.identifier} - ${createdIssue.title}`);
            return createdIssue;
     ...
     */
    public async updateIssueStatus(issueId: string, statusName: string): Promise<LinearIssue> {
        try {
            ...
            console.log(`✅ Updated Linear issue ${updatedIssue.identifier} status to: ${statusName}`);
            return updatedIssue;
     ...
     */
    public async addCommentToIssue(issueId: string, body: string): Promise<LinearComment> {
        try {
     ...
     */
    public async findDuplicateIssue(feedback: ProcessedFeedbackData): Promise<LinearIssue | null> {
        try {
            ...
            if (feedback.screenshotData?.text) {
                const cleanText = feedback.screenshotData.text.substring(0, 50).replace(/"/g, '');
     ...
     */
    public async getIssueStatusByName(statusName: string): Promise<LinearIssueStatus> {
        ...
        try {
            const status = await this.mcpGetIssueStatus(statusName, this.config.teamId);
            this.statusCache.set(normalizedName, status);
            return status;
        } catch (error) {
            throw new Error(`Failed to get Linear issue status '${statusName}': ${error}`);
        }
    }
    ...
    private async mcpCreateIssue(request: LinearCreateIssueRequest): Promise<LinearIssue> {
        // This will be implemented to use the MCP Linear tools
        // For now, return a mock response to satisfy TypeScript
        throw new Error('MCP Linear integration not yet connected. Please implement MCP tool integration.');
    }

    private async mcpUpdateIssue(request: LinearUpdateIssueRequest): Promise<LinearIssue> {
        // This will be implemented to use the MCP Linear tools
        throw new Error('MCP Linear integration not yet connected. Please implement MCP tool integration.');
    }

    private async mcpCreateComment(request: LinearCreateCommentRequest): Promise<LinearComment> {
        // This will be implemented to use the MCP Linear tools
        throw new Error('MCP Linear integration not yet connected. Please implement MCP tool integration.');
    }

    private async mcpListIssues(params: LinearIssueSearchParams): Promise<LinearIssue[]> {
        // This will be implemented to use the MCP Linear tools
        throw new Error('MCP Linear integration not yet connected. Please implement MCP tool integration.');
    }

    private async mcpGetTeam(teamId: string): Promise<LinearTeam> {
        // This will be implemented to use the MCP Linear tools
        throw new Error('MCP Linear integration not yet connected. Please implement MCP tool integration.');
    }

    private async mcpListIssueStatuses(teamId: string): Promise<LinearIssueStatus[]> {
        // This will be implemented to use the MCP Linear tools
        throw new Error('MCP Linear integration not yet connected. Please implement MCP tool integration.');
    }

    private async mcpGetIssueStatus(statusName: string, teamId: string): Promise<LinearIssueStatus> {
        // This will be implemented to use the MCP Linear tools
        throw new Error('MCP Linear integration not yet connected. Please implement MCP tool integration.');
    }

    private async mcpListIssueLabels(teamId: string): Promise<LinearIssueLabel[]> {
        // This will be implemented to use the MCP Linear tools
        throw new Error('MCP Linear integration not yet connected. Please implement MCP tool integration.');
    }

    private async mcpListProjects(teamId: string): Promise<LinearProject[]> {
        // This will be implemented to use the MCP Linear tools
        throw new Error('MCP Linear integration not yet connected. Please implement MCP tool integration.');
    }
    ...
    private prepareIssueFromTestFlight(
        ...
        if (isCrash) {
            ...
            priority = feedback.crashData?.exceptionType?.toLowerCase().includes('fatal') ? 1 : 2;
        }

        // Combine labels
        const baseLabels = isCrash ? this.config.crashLabels : this.config.feedbackLabels;
        const allLabels = [...this.config.defaultLabels, ...baseLabels, ...additionalLabels];
    ...
    private async resolveLabelIds(labelNames: string[]): Promise<string[]> {
        try {
            ...
            if (missingLabels.length > 0) {
                console.warn(`Labels not found in Linear workspace: ${missingLabels.join(', ')}`);
            }
     ...
     */
    private async addTestFlightCommentToIssue(issueId: string, feedback: ProcessedFeedbackData): Promise<LinearComment> {
