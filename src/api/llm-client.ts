...
export class LLMClient {
	 ...
	 * Uses llm-bridge for provider interoperability
	 */
	public async makeRequest(
		...
		for (const provider of providers) {
			try {
				// Convert request to target provider format using llm-bridge
				const sourceProvider = "openai"; // Assume our request format is OpenAI-like
				const providerRequest = translateBetweenProviders(
					sourceProvider,
					provider,
					request,
				);
				...
				const response = await this.callProviderHTTP(
					provider,
					providerRequest,
					options,
				);
				...
				return this.parseProviderResponse(response, provider);
			} catch (error) {
	...
	private async processRequest(
		...
		const adapter = detectProvider(provider);
		...
		const universalResponse = adapter.toUniversal(
			providerResponse,
			sanitizedRequest,
		);
	 ...
	 */
	private emitResponse(universalResponse: any): LLMResponse {
		return {
			content: universalResponse.content,
			usage: universalResponse.usage,
			model: universalResponse.model,
	...
	private async validateCostLimits(
		...
		const inputTokens = this.estimateTokens(
			JSON.stringify(universalRequest.messages),
		);
	 ...
	 */
	private sanitizeUniversalRequest(
		request: any,
	): any {
		return {
			...request,
			messages: request.messages.map((msg) => ({
				...
				content: msg.content.map((content) => ({
					...content,
					text: content.text
						? sanitizeDataForLLM(content.text, this.config)
						: content.text,
				})),
			})),
		};
	...
	private async callProviderHTTP(
		...
		try {
			...
			if (!response.ok) {
				...
				const error = translateError(provider, errorInfo);
				throw new Error(`${error.type}: ${error.message}`);
			}

			return (await response.json()) as Record<string, unknown>;
		} catch (error) {
			const normalizedError = translateError(provider, error);
...
`;
		...
		userPrompt +=
			"Please analyze this TestFlight feedback and provide an enhanced issue following the JSON format specified above.";
